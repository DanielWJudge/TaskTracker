#!/usr/bin/env python3
"""
Fixed tasker.py – one‑task‑at‑a‑time CLI tracker with --plain and backlog support.
All load/save operations call `load()` once to avoid data loss.
"""
...
# ===== Styling helpers =====
RESET = "\033[0m"; CYAN = "\033[96m"; GRAY = "\033[90m"; BOLD = "\033[1m"
...
def cmd_done(args):
    ...
    print(f"{emoji('complete')} Completed: {today['todo']}")
    today["todo"] = None; save(data); cmd_status(args)

def cmd_status(_):
    today = ensure_today(load()); today_str = today_key()
    print(f"\n=== TODAY: {today_str} ===")
    for it in today["done"]:
        ts = it['ts'].split('T')[1]; print(f"{emoji('added')} {it['task']} [{ts}]")
    if not today["done"]:
...
def cmd_newday(_):
    data = load(); ensure_today(data); save(data)
...
def cmd_backlog(args):
    ...
    if args.subcmd == "add":
        ...
        save(data); print(f"{emoji('backlog_add')} Backlog task added: {args.task}")
        ...
        task = today["backlog"].pop(0); today["todo"] = task['task']; save(data)
...
def build_parser():
    ...
    a = sub.add_parser("add"); a.add_argument("task", nargs="+"); a.set_defaults(func=cmd_add)
    sub.add_parser("done").set_defaults(func=cmd_done)
    sub.add_parser("status").set_defaults(func=cmd_status)
    sub.add_parser("newday").set_defaults(func=cmd_newday)
    b = sub.add_parser("backlog"); b_sub = b.add_subparsers(dest="subcmd", required=True)
    b_a = b_sub.add_parser("add"); b_a.add_argument("task", nargs="+"); b_a.set_defaults(func=cmd_backlog)
...
def main():
    ...
    if args.store: STORE = Path(args.store)
