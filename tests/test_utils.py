...
import tasker
from tasker import (
    format_backlog_timestamp, print_backlog_list, style, emoji,
    today_key, Config
)
...
class TestPrintBacklogList:
    ...
    def test_single_item_backlog(self, capsys):
        ...
        backlog = [
            {"task": "Single task", "ts": "2025-05-30T14:30:00"}
        ]
        print_backlog_list(backlog)
    ...
    def test_backlog_with_invalid_timestamp(self, capsys):
        ...
        backlog = [
            {"task": "Task with bad timestamp", "ts": "invalid-timestamp"}
        ]
        print_backlog_list(backlog)
    ...
    def test_all_emoji_keys(self):
        ...
        try:
            ...
            for key, expected_emoji in expected_emojis.items():
                result = emoji(key)
                assert result == expected_emoji, f"Emoji for '{key}' should be '{expected_emoji}', got '{result}'"
        finally:
            tasker.USE_PLAIN = original_plain
...
class TestTimestampFormatting:
    ...
    def test_edge_case_timestamps(self):
        ...
        test_cases = [
            ...
            ("2025-02-29T12:00:00", "[02/29 12:00]"),  # Leap year (2025 is not leap, but test parsing)
        ]
...
class TestDisplayIntegration:
    ...
    def test_print_backlog_uses_format_timestamp(self, capsys):
        ...
        backlog = [
            {"task": "Test task", "ts": "2025-05-30T14:30:00"}
        ]
